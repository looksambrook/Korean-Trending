"""Register and bound one actual offline C0/CR/CF text proxy execution.
...
from creation_model_backend import model_identity, run_experiment, sha_file, validate_settings, write_json
...
def run(spec_path,output_dir,db,retry_of=None,retry_reason=None):
    ...
    spec={"objective":"Actual bounded developer local text proxy C0 CR CF inference and low-rank training",
        "stage":"model_experiment","parameters":{"settings":options,"versions":versions,"scope":"developer_local_text_proxy_pilot"},
        "data_snapshots":[{"id":"experiment_input","sha256":sha_file(source)},{"id":"actual_model_snapshot","sha256":identity["snapshot_sha256"]}],
        "code_hashes":{"backend":sha_file(ROOT/"src/creation_model_backend.py"),"runner":sha_file(Path(__file__))},
    ...
    try:
        ...
        request={"model_path":data["model_path"],"revision":data["revision"],"cases":data["cases"],
            ...
            "registered_run_id":registered["run_id"],"identity":identity}
        ...
        log.finish(registered["run_id"],status="succeeded" if result["status"]=="completed" else "failed",
            notes=f"Developer local text proxy {result['status']}; actual optimizer_steps={result.get('optimizer_steps',0)}; generation_calls={result.get('generation_calls',0)}. This is not native multimodal or validated content-quality evidence.",
