...
아직 확인이 필요한 것은 실제 입력 자료의 위치·사용 범위, 최종 제작물 형식, 사용 가능한 GPU/서버와 비용 상한이다. 모델·평가자·기간·표본 규모·투고처도 미정이다. 유료 연산과 사람 평가 모집은 확정된 범위를 따른다. 특정 학술지의 게재를 보장하거나 제안 표본 수를 KCI 공식 조건처럼 쓰지 않는다.
...
현재까지 생성/FT 비교 결과는 없다. 이전의 YouTube 수집·규칙 기반 계열 보고서와 로컬 모델 메모리 부족 기록은 과거 발견 작업의 기록이며, 이번 멀티모달 제작 비교의 성능 결과로 재분류하지 않는다.
